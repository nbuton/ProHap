...
parser = argparse.ArgumentParser(
    description="Reads a FASTA file, aggregated all the duplicate sequences into one entry."
)

parser.add_argument(
    "-i", dest="input_file", required=True, help="input FASTA file", metavar="FILE"
)

parser.add_argument(
    "-o", dest="output_file", required=True, help="output FASTA file", metavar="FILE"
)
...
for protein in all_proteins.values():
    ...
    if ("hap" in protein["tag"]) or ("var" in protein["tag"]):
        matching_proteins = (
            protein["description"]
            .split("matching_proteins:", 1)[1]
            .split(maxsplit=1)[0]
            .split(";")
        )
        rfs = (
            protein["description"]
            .split("reading_frame:", 1)[1]
            .split(maxsplit=1)[0]
            .split(";")
        )
        protein_start = (
            protein["description"].split("start:", 1)[1].split(maxsplit=1)[0]
        )
        if "position_within_protein" in protein["description"]:
            seq_position = (
                protein["description"]
                .split("position_within_protein:", 1)[1]
                .split(maxsplit=1)[0]
            )
        else:
            seq_position = "0"
    ...
    nearest_idx = bisect.bisect_left(
        KeyWrapper(result_proteins, key=lambda x: x["hash"]), seq_hash
    )
    if (
        len(result_proteins) > nearest_idx
        and result_proteins[nearest_idx]["hash"] == seq_hash
    ):
        ...
        result_proteins.insert(
            nearest_idx,
            {
                "hash": seq_hash,
                "matching_proteins": [matching_proteins],
                "split_sequences": [protein["accession"]],
                "tags": [protein["tag"]],
                "sequence": seq,
                "start": [protein_start],
...
for i, protein in enumerate(result_proteins):
    ...
    description = (
        ...
        + ";".join(protein["seq_position"])
        + " start:"
        + ";".join(protein["start"])
        + " matching_proteins:"
        + ";".join(matching_proteins)
        + " reading_frame:"
        + ";".join(rfs)
        + " split_sequences:"
        + ";".join(protein["split_sequences"])
    )
    ...
    if ("generic_cont" in protein["tags"]) or ("generic_sp" in protein["tags"]):
        ...
        "generic_manual" in protein["tags"]
    ):  # in case some sequences were added to the fasta manually
        tag += "manual"
    ...
    outfile.write(
        ">" + tag + "|prot_" + hex(i)[2:] + "|" + description.replace("\n", "") + "\n"
    )
