...
def is_valid_file(parser, arg):
    if not os.path.exists(arg):
        ...
        return gzip.open(arg, "rt")  # open(arg, "r")  # return an open file handle
...
parser.add_argument(
    ...
    required=True,
    help="list of transcript IDs, provided in a CSV file",
    metavar="FILE",
)
...
parser.add_argument(
    ...
    required=False,
    type=int,
    help="flag: require annotation of the start codon, set to 0 to disable; default: 1",
    default=1,
)
...
parser.add_argument(
    ...
    required=False,
    type=int,
    help="flag (0 or 1): ignore variation in the UTR sequences, do not add UTR translation to proteins; default: 1",
    default=1,
)

parser.add_argument(
    "-skip_start_lost",
    dest="skip_start_lost",
    required=False,
    type=int,
    help="flag (0 or 1): ignore haplotypes where the start codon is lost; default: 1",
    default=1,
)
...
parser.add_argument(
    ...
    dest="force_rf",
    required=False,
    help="Force the most likely reading frame when start codon is not annotated or lost due to mutation, set to 0 to disable; default: 1",
    default=1,
)
...
parser.add_argument(
    ...
    required=False,
    type=int64,
    help="end location of the 1st pseudoautosomal region on chromosome X; default: 2,781,479",
    default=2781479,
)
...
parser.add_argument(
    ...
    required=False,
    type=int64,
    help="start location of the 2nd pseudoautosomal region on chromosome X; default: 155,701,383",
    default=155701383,
)
...
parser.add_argument(
    ...
    required=False,
    type=float,
    help="Minimum frequency of a haplotype to be reported in the result (specify -1 to use count threshold instead); default: -1",
    default=-1,
)
...
parser.add_argument(
    ...
    required=False,
    type=int,
    help="Minimum count of occurrences of a haplotype to be reported in the result (used only if frequency threshold is not specified); default: 0",
    default=0,
)
...
transcript_list = transcript_df[transcript_df["chromosome"] == args.chromosome][
    "transcriptID"
].tolist()
...
for transcript_id in transcript_list:
    try:
        ...
        print(e)
        print(
            f"Transcript {transcript_id} not found in this chromosome verify that it is present in the gtf"
        )
        print("Maybe raise error here?")
        continue
    if args.require_start:  # start codon annotation is required - check if present
        start_codons = [
            sc
            for sc in annotations_db.children(
                feature, featuretype="start_codon", order_by="start"
            )
...
vcf_colnames = parse_vcf(
    all_transcripts, args.input_vcf, annotations_db, args.min_af, args.tmp_dir
)

# keep only the samples that are in the metadata table
sample_ids = [
    sample for sample in vcf_colnames if (sample in samples_df["Sample name"].tolist())
]
...
if len(vcf_colnames) == 0:
    print(
        ("Chr " + args.chromosome + ":"),
        "VCF file is empty, creating empty output files.",
    )
    ...
    gene_haplo_df = get_gene_haplotypes(
        all_transcripts,
        sample_ids,
        args.tmp_dir,
        args.log_file,
    ...
    haplo_results = process_haplotypes(
        all_transcripts,
        gene_haplo_df,
        all_cds,
        annotations_db,
        args.chromosome,
        args.haplo_id_prefix,
        args.force_rf,
        args.threads,
        folder_res,
        args.min_foo,
    ...
    for i, seq in enumerate(result_sequences):
        ...
        output_fasta_file.write(
            ">" + args.fasta_tag + "|" + accession + "|" + description + "\n"
        )
